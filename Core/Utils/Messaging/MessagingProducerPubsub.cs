    ...
    IVault vault) : IMessagingProducer, ISystemProducer
{
    ...
    public  async Task Produce(string topic, string key, string message)
    {
        _topic = CheckAndUpdateSuffix(topic);
        await PostWithRetry(_topic, key, message);
    }

    private async Task Post(string topic, string key, string message)
    {
        ...
        await publisher.PublishAsync(topicName, new[] { new PubsubMessage {MessageId = key, Data = ByteString.CopyFrom(data) } });
        logger.LogDebug("MessagingProducerPubsub Produce: {info}", $"Message sent to {topic}");
    }
    
    private async Task PostWithRetry(string topic, string key, string message,int maxRetries = 3)
    {
        ...
        while (loop)
        {
            ...
            {
                isRetry = true;
                logger.LogError("MessagingProducerPubsub Produce: {info}", $"Error= {ex?.Message}");
                logger.LogInformation("MessagingProducerPubsub Produce: {info}", $"Retrying to send message to {topic}");
    ...
    private async Task InsertLog(string key, string message, int retries)
    {
        ...
        var p = new Message<object>(appId: appId, key: key, payLoad: message, retry: retries);
